在刚开始使用babel的时候，相信很多同学应该和我一样，对于babel的使用配置一知半解，几个babel相关的包@babel/core，@babel/cli，babel-loader，@babel/polyfill，@babel/plugin-transform-runtime，@babel/runtime何时引入又有什么作用和区别感到疑惑。这篇文章的目的就是帮助还没使用或刚刚使用babel的同学快速了解这些内容，游刃有余的使用babel这个强大的工具。  
...
## 配置 
使用方法很简单，但只是这样还不够，现在执行babel编译，会发现编译是成功的，但是编译后的内容和编译前没有什么区别，那是因为我们没有告诉babel要怎么去编译，编译哪些内容，我们需要一个配置文件来告诉babel如何工作。   
配置文件的方式有以下几种：
1. 在package.json中设置babel字段。
2. .babelrc文件或.babelrc.js
3. babel.config.js文件  
   
第1种方式不用创建文件，package.json加入babel的配置信息就行。第二种.babelrc和.babelrc.js是同一种配置方式，只是文件格式不同，一个是json文件，一个是js文件。   
...
## Plugins和Presets
有了配置文件，接下来就是要通过配置文件告诉babel编译哪些内容，然后还要引入对应的编译插件（Plugins），比如上面讲到的箭头函数的转换需要的是[@babel/plugin-transform-arrow-functions](https://www.babeljs.cn/docs/babel-plugin-transform-arrow-functions)这个插件，我们通过npm安装这个插件包之后在配置里面进行设置。
```shell
...
```
现在我们代码中的箭头函数都会被编程成普通函数，但是有个问题，我们总不能一个个的引入这些插件，来对应转化我们用到的每个新特性，这也太麻烦了，于是有了一个东西叫做预设（Presets）。   
...
## preset-env
preset虽然已经大大简化了我们的使用，但是如果我们还想使用更新一些的语法，比如es2016的**（相当于pow()）,es2017的async/await等等，我们就要引入@babel/preset-es2016，@babel/preset-es2017之类的，而且随着js语法的更新，这些preset会越来越多。于是babel推出了babel-env预设，这是一个智能预设，只要安装这一个preset，就会根据你设置的目标浏览器，自动将代码中的新特性转换成目标浏览器支持的代码。   
...
**注意1：即使不设置targes，browserlist也会有一个默认值，规则为 > 0.5%, last 2 versions, Firefox ESR, not dead。**   
**注意2：官方推荐使用preset-env。**

## plugin-transform-runtime和runtime

当我在用babel编译时，有些功能需要一些工具函数来辅助实现，比如class。
```javascript
class People{
}
...
```
编译后的代码中，_classCallCheck就是一个辅助功能实现的工具函数。如果多个文件中都用到了class，每一个文件都生成一个工具函数，最后就会产生大量重复代码，平白增加文件体积。而plugin-transform-runtime就是为了解决这个问题，这个插件会将这些工具函数转换成引入的形式。   
```
...
```
_classCallCheck2这个工具函数已经变成从一个npm包中引入，不会再产生单独的工具函数代码。但是可以看到这些工具函数是从@babel/runtime这个包中引入，所以要安装@babel/runtime这个依赖包，在项目打包的时候才不会报错。
```shell
npm i @babel/runtime
```
...
## babel-polyfill
babel可以转化一些新的特性，但是对于新的内置函数（Promise,Set,Map），静态方法（Array.from,Object.assign），实例方法（Array.prototype.includes）就需要babel-polyfill来解决，babel-polyfill会完整模拟一个 ES2015+环境。比如你的代码中用到了Array.from，但是目标浏览器不支持Array.from，引入babel-polyfill就会给Array添加一个from方法，代码执行的时候使用的Array.from其实是babel-polyfill模拟出来功能，这样虽然污染了Array的静态方法，但是确实实现了兼容。
...
但是这种方式已经被废弃不推荐使用，因为@babel/polyfill体积比较大，整体引入既增加项目体积，又污染了过多的全局环境，所以更推荐使用preset-env来按需引入polyfill。
```javascript
// babel.config.js
module.exports = {
    presets: [
...
```
corejs 是一个给低版本的浏览器提供接口的库，也是polyfill的核心，此处指定的是引入的corejs的版本，需要通过npm安装指定版本的corejs库作为生产依赖。 
```shell
npm i core-js@2
```
之后执行babel编译可以看到如下效果：
...
```
可以看到在使用Array.from之前，提前从core-js引入了相应的polyfill。  

## plugin-transform-runtime和babel-polyfill的讨论
上文说了plugin-transform-runtime主要是负责将工具函数转换成引入的方式，减少重复代码，而babel-polyfill则是引入相关文件模拟兼容环境。babel-polyfill有一个问题就是引入文件会污染全局变量，其实plugin-transform-runtime也提供了一种runtime的polyfill。   
我们将配置文件修改一下。
```javascript
module.exports = {
...
```
这里的corejs和presets里设置的corejs是不同的，这个地方的corejs是指定了一个叫runtime-corejs库的版本，需要用npm安装对应的包。
```
...
```
可以看到，此方法和使用babel-polyfill的区别是，并没有改变Array.from,而是创建了一个_from来模拟Array.from的功能，调用Array.from其实是调用了_from，这样做的好处很明显就是不会污染Array上的静态方法from，plugin-transform-runtime提供的runtime形式的polyfill都是这种形式。   
...
然后我就想，既然这种形式不会污染变量和内置对象的静态方法，那当然能用就用这种了，但是群里问了一下后，大佬们给出了一个看法（感谢justjavac和小时哥的说明）。
